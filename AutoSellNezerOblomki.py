...
class MinecraftSellerApp:
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("Minecraft AH Seller BugFix2.5")
    ...
    def _calibration_thread(self):
        ...
        try:
            ...
            prompts = { 11: "Наведите на ПЕРВУЮ кнопку в /ah (например, 'обновить лоты')", 12: "Наведите на ВТОРУЮ кнопку в /ah (например, 'подтвердить')" }
    ...
    def _type_command_via_clipboard(self, command):
        ...
        try: original_clipboard_content = self.root.clipboard_get()
        ...
        self.root.clipboard_clear()
        if original_clipboard_content: self.root.clipboard_append(original_clipboard_content)
    ...
    def _persistent_log_watcher(self):
        ...
        if not os.path.exists(log_file): self.log_message(f"[Наблюдатель]: ОШИБКА: Лог-файл не найден!", "error"); return
        try:
    ...
    def _anti_afk_thread(self):
        self.log_message("--- [Анти-АФК]: Запущен. ---")
        while self.is_running:
            time.sleep(30)
            if not self.is_running: break

            self.log_message("[Анти-АФК]: Захватываю управление для выполнения действий...")
            with self.action_lock:
                self.log_message("[Анти-АФК]: Управление захвачено. Выполняю...")
                
               
                if self.anti_afk_forward:
                    self.log_message("[Анти-АФК]: Движение ВПЕРЕД.")
                    pydirectinput.keyDown('w'); time.sleep(0.1); pydirectinput.keyUp('w')
                else:
                    self.log_message("[Анти-АФК]: Движение НАЗАД.")
                    pydirectinput.keyDown('s'); time.sleep(0.1); pydirectinput.keyUp('s')
                ...
                self.log_message("[Анти-АФК]: Действия завершены. Возвращаю управление.")
        self.log_message("--- [Анти-АФК]: Остановлен. ---")
