...
class MinecraftSellerApp:
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("Oblomli/tripki seller by velenok ds 4596036")
        ...
        calib_frame = ttk.LabelFrame(main_frame, text="3. Калибровка координат")
    ...
    def load_settings(self):
        try:
            ...
            if len(coords_from_file) == 10:
                self.coordinates = {int(k): pyautogui.Point(v[0], v[1]) for k, v in coords_from_file.items()}; self.is_calibrated = True
                self.log_message(f"Настройки и координаты загружены из {self.settings_file}."); self.calibration_status_label.config(text="Статус: Координаты загружены"); self.start_button.config(state="normal")
            else: self.log_message(f"Настройки загружены, но координаты отсутствуют. Нужна калибровка."); self.calibration_status_label.config(text="Статус: Требуется калибровка")
            self.setup_hotkey()
    ...
    def _calibration_thread(self):
        ...
        try:
            ...
            slots_to_calibrate = [10] + list(range(1, 10))
            for slot_num in slots_to_calibrate:
                self.calibration_status_label.config(text=f"Наведите на слот [{slot_num}] и НАЖМИТE ПРАВЫЙ CTRL")
                keyboard.wait('right ctrl'); pos = pyautogui.position()
                self.coordinates[slot_num] = pos; self.log_message(f"Слот [{slot_num}] откалиброван: {pos}"); time.sleep(0.5)
    ...
    def start_process(self):
        if not self.is_calibrated: messagebox.showerror("Ошибка", "Сначала нужно откалибровать координаты!"); return
        ...
        try:
            ...
            while self.is_running:
                ...
                for i in range(0, 9, 3):
                    ...
                    self.log_message(f"[Логика]: Ожидаю 3 продажи из очереди...")
                    sales_needed = 3
                ...
                self.log_message("--- [Логика]: Все 9 предметов проданы. Начинаю цикл заново. ---")
                time.sleep(2) 
